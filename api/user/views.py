...
def generate_session_token(lenght=10):
    ...
    ] + [str(i) for i in range(10)]) for _ in range(lenght))
    ...
    try:
        ...
        if user.check_password(password):
            ...
            if user.session_token != "0":
                ...
                user.save
                return JsonResponse({'error': "privious session exist"})
...
def signout(request, id):
    ...
    return JsonResponse({'seccess': 'Logout seccess for user'})
...
class UserViewSet(viewsets.ModelViewSet):
    ...
    #set all permisions to not admin
